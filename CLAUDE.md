...
- [ ] 도식이 아직 없는 장들 — `data` 3·6·7·9·10장, `web3` 5·8·10·11·12·13장.
      `figure` 블록에 인라인 SVG를 넣고, 색은 반드시 `currentColor` + `fill-opacity`로 표현합니다
      (테마 전환에 자동 대응). 넣은 뒤에는 실제 렌더링을 확인해 라벨 겹침·`viewBox` 이탈을 잡으세요 —
      SVG는 빌드가 잡아주지 않습니다. SVG 안에는 마크다운 링크(`[n장](/경로)`)를 쓸 수 없습니다.
