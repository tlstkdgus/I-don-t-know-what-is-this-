...
현재: `web3`(13장 완료), `data`(13장 완료 — 파트 4까지 전부),
...
- **브랜치**: `main`은 항상 배포 가능한 상태로 유지합니다. 성격이 다른 작업은 브랜치를 분리해서
  `{종류}/{짧은-kebab-설명}` 이름을 씁니다.
  - `content/` — 챕터·이미지 등 콘텐츠 작업 (예: `content/beginner-friendly`, `content/images`)
  - `docs/` — README·CLAUDE.md 등 문서/카피 (예: `docs/home-copy`)
  - `feat/` — 새 기능 (예: `feat/toc`)
  - `fix/` — 버그 수정 중 브랜치가 필요할 만큼 범위가 있는 것
- **큰 작업 단위 하나가 끝날 때마다 커밋하고 바로 푸시**합니다. 다 끝날 때까지 모아뒀다가
  한 번에 커밋하지 않습니다. 서로 무관한 작업(예: 이미지 추가와 콘텐츠 톤 재작성)은 같은
  브랜치에 섞지 않고 브랜치 자체를 분리합니다.
- **커밋 전 항상 `npm run build`로 검증**합니다 — 타입 에러나 정적 생성 실패를 미리 잡습니다.
- **커밋 메시지**: `type: 한국어 요약` 형식이고, 본문은 필요할 때만 `-` 불릿으로 "무엇을 왜"
  바꿨는지 적습니다(코드가 이미 설명하는 내용은 반복하지 않음).
  - `type`: `feat`(새 기능) · `fix`(버그 수정) · `docs`(문서·콘텐츠) · `refactor`(동작은 그대로,
    구조만 변경) · `merge`(브랜치 병합)
  - `Co-Authored-By` 트레일러는 붙이지 않습니다 — 이 저장소 기존 커밋들의 스타일을 따릅니다.
- **`main`에 직접 커밋하지 마세요. 크기와 무관하게 항상 브랜치를 먼저 만듭니다.**
  오타 하나를 고칠 때도 그렇습니다. 예외는 사용자가 "바로 push해"라고 명시할 때뿐입니다.
  (이전 규칙에는 "사소한 수정은 main에 바로"라는 예외가 있었는데, 그 판단이 계속
  느슨해져서 리뷰 없이 올라가는 커밋이 생겼습니다. 그래서 예외를 없앴습니다.)
- **병합은 사용자가 GitHub PR에서 직접 합니다.** 브랜치를 푸시하는 데까지가 기본 범위이고,
  명시적으로 요청받지 않으면 PR을 임의로 병합하지 않습니다.
...
| `diagram` | `name` `caption?` | SVG 도식. `components/figures/`의 FIGURES 키 |
| `hr` | — | 구분선 |
...
**도식 추가**
```
1. components/figures/{data,web3}.tsx 에 컴포넌트 작성
2. components/figures/index.tsx 의 FIGURES 에 등록
3. 본문에 { t: "diagram", name: "키", caption: "..." } 삽입
4. 실제 렌더링을 눈으로 확인 (빌드는 레이아웃 문제를 못 잡습니다)
```

도식은 `Demos`와 같은 레지스트리 패턴이지만 **서버 컴포넌트**입니다 — 정적이라
클라이언트로 나가는 JS가 없습니다. `"use client"`를 붙이지 마세요.
...
- [x] `data` 파트 4 「운영과 신뢰」 — 11 데이터 계보 · 12 데이터 계약 · 13 개인정보와 데이터 최소화.
      13장은 「법률 자문이 아님」을 본문 최상단과 말미에 명시했고, 인용한 조항·지침은 전부
      `legend` 블록에 출처를 남겼습니다. **개정이 잦은 영역이라 갱신 시 반드시 원문을 다시 확인할 것.**
- [ ] `data` 3장 테이블 포맷 현황(Iceberg/Delta/Hudi) 재확인 — 2026년 7월 기준으로 적었습니다
- [x] 도식 — 26개 장 전부 보유(총 31개). 부품과 규칙은
      `components/figures/primitives.tsx` 주석에 있습니다. 요약하면:
      좌표를 손으로 세지 말고 `row()`/`stack()`을 쓸 것, 색은 하드코딩 hex 대신 `C.*`와
      의미색(`tone`)을 쓸 것, 강조는 테두리와 배경을 **함께** 바꿀 것(테두리만 바꾸면 안 보입니다),
      주 라벨과 보조 라벨의 크기를 분리할 것. SVG 안에는 마크다운 링크를 쓸 수 없습니다.
      **넣은 뒤 반드시 실제 렌더링을 눈으로 확인하세요** — 타입 체크는 라벨 겹침이나
      `viewBox` 이탈을 잡지 못합니다. 실제로 이 방식으로만 잡힌 버그가 9건이었습니다.
      한 줄에 들어가는 한글은 `font-size` 11.5에서 대략 22자(폭 264px)가 상한입니다.
